...
- Red-flag: multiple structs defining the same feature, and they need to be synchronized for the feature to work: f.ex. in config a dict of dl-components with role->keyword pairings, and then a SEPARATE dict with keyword->data that actually defines things. Directly just role->data.
- Red-flag: A component's type, existence, and internal state are private to its own file. No file outside a component's definition may import it, name it, or branch on its presence. All routing between classes happens exclusively through the config's ideclarations and the shared namespace. If a feature requires coordination between classes, the coordination mechanism must be expressible as named keys — not as logic in any logic module or any orchestrating layer. For example, seeing a component's class name or a feature-flag check (such as, 'if "roi" in self.components') anywhere outside the roi-component's own file is a red flag.
