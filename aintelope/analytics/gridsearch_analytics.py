...
def gridsearch_analytics(cfg: DictConfig) -> None:
    ...
    groupby_cols = [
        col
        for col in df.columns
        if (use_separate_models_for_each_experiment and col == "experiment_name")
        or (
    ...
    df_params_and_results = df[
        [col for col in df.columns if col in groupby_cols or col in score_cols]
    ]
    ...
    averages_per_parameter_combination = (
        averages_per_parameter_combination.drop_duplicates()
    )
    ...
    best_parameters_by_score = averages_per_parameter_combination.loc[
        averages_per_parameter_combination.groupby(grouping_dims)[
            "test_averages.Score"
        ].idxmax()
    ]
    best_parameters_by_sfella_score = averages_per_parameter_combination.loc[
        averages_per_parameter_combination.groupby(grouping_dims)[
