...
def create_widget(
    ...
    # File browser -> Entry + Browse button
    if value_type == "file":
        var = tk.StringVar(value=str(value))
        frame = tk.Frame(parent)
        entry = tk.Entry(frame, textvariable=var, width=ENTRY_WIDTH, font=font)
        ...
        tk.Button(
            frame, text="Browse…", command=browse, font=(FONT_FAMILY, FONT_SIZE)
        ).pack(side=tk.LEFT, padx=5)
        entry.bind("<FocusOut>", lambda e: on_change(var.get()))
        entry.bind("<Return>", lambda e: on_change(var.get()))

        return frame, lambda v: var.set(str(v))
