...
def create_widget(
    ...
    if value_type == "bool":
        ...
        canvas = tk.Canvas(frame, width=size, height=size, highlightthickness=1, highlightbackground="black")
        canvas.pack(side=tk.LEFT)
        
        def draw_checkbox():
            canvas.delete("all")
            canvas.create_rectangle(2, 2, size-2, size-2, outline="black", fill="white")
            if var.get():
                # Draw checkmark
    ...
    if isinstance(range_or_choices, list) and len(range_or_choices) == 2:
        ...
        btn_up = tk.Button(button_frame, text="▲", command=increment_value, 
                           width=2, font=(FONT_FAMILY, FONT_SIZE))
        btn_up.pack(side=tk.TOP)
        
        btn_down = tk.Button(button_frame, text="▼", command=decrement_value,
                             width=2, font=(FONT_FAMILY, FONT_SIZE))
